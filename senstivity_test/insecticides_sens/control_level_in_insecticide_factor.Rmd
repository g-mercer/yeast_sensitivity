...
# create a dataframe where standard df has controls as insecticide level and conc of 0
only_controls <- model_df_anomalies_removed [grep(pattern = "control", x = model_df_anomalies_removed$sample), ]

model_df_controls_zero <- model_df_anomalies_removed [-grep(pattern = "control", x = model_df_anomalies_removed$sample), ]

only_controls$concentration <- "0"

model_df_controls_zero <- rbind(model_df_controls_zero, only_controls)
```
...
First option (model_df_controls_zero) is where controls are within control level in insecticide factor and 0 level in concentration factor.
Second (model_df_assigned_controls) is where controls have been distributed to each insecticide level within insecticde factor and 0 level in concentration factor. Third (model_df_anomalies_removed) is where control is a level within insecticide factor and have been distributed to concentration levels.  

```{r}
# using each dataset as input 
...
# reorder concentration factor so "0" (control) is at the bottom of ordinal variable
# model_df_anomalies_removed$concentration <- factor(model_df_anomalies_removed$concentration, order = TRUE,
#                                                    levels = c("0", "0.001", "0.01", "0.1", "1", "5"))
...
An important question is what does the intercept of my model represent? For a simple linear regression of a categorical variable with two levels the intercept would be one of the levels, with the other level contrasted to this level. Eg. male and female levels in sex categorical variable in car package salaries example data set ([link](http://www.sthda.com/english/articles/40-regression-analysis/163-regression-with-categorical-variables-dummy-coding-essentials-in-r/#example-of-data-set)). For more complex models like mine this isn't the case. Example on sthda has same issue where it isn't obvious what the intercept relates to. 

Linear mixed model with control as its own level in concentration
...
mixed.lmer_trans <- lmer(auc_l ~ insecticide + concentration + insecticide*concentration + (1|day), data = transformed_data)
...
# model and term significance.
full.lmer_trans <- lmer(auc_l ~ insecticide + concentration + (insecticide*concentration) + (1|day), data = transformed_data, REML = FALSE)

reduced.lmer_trans <- lmer(auc_l ~ insecticide + concentration + (1|day), data = transformed_data, REML = FALSE)
...
Model residuals look closer to a normal distribution and shapiro wilks test now fails. 18% chance of observing this result if the model residuals were normally distributed.
