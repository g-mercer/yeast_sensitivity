...
```{r}
replicates_of_interest <- c("time", "chlorpyrifos-0.1-7", "tefluthrin-0.1-7", "thiacloprid-5-4", "thiacloprid-5-5", "control-5-7", "thiacloprid-1-3", "control-1-8", "imidacloprid-0.01-8", "flupyradifurone-0.01-8", "clothianidin-0.1-8", "control-1-5", "thiacloprid-1-6", "malathion-5-3", "acetamiprid-5-6")
...
Need to remove outliers through a combination of PCA, sigma values and visual inspection of plots. Samples are removed if they fit the following three criteria: not bound by PCA y = -4 and x = 5, sigma > 0.05 and obvious error from visual inspection of plots. Two controls are anomalies but we require them for model (and associated incomplete block design) so keep for now.

```{r}
# put control back into anomalies to see effect , "control-5-7"
anomalies <- c("chlorpyrifos-0.1-7", "tefluthrin-0.1-7", "thiacloprid-5-4", "thiacloprid-5-5", "thiacloprid-1-3")
...
Plot means and 95% CI for each insecticide to get a quick visual overview of any potential effect. Thiacloprid might have an effect at 5mM. Sulfoxaflor and cypermethrin were removed from analysis at this stage due to evaporation and solubility issues respectively. Controls are shared in the by insecticide plots below.
...
These are the only two models that mean anything biologically. Insecticide and concentration only mean something when taken together. What are the assumptions of linear regression? Normally distributed and homogeneity of variance. Test these assumptions. 
...
model_2_metrics <- augment(model_2)

shapiro_test(model_2_metrics$.resid)
```
...
Plot histogram of dependent variable, kernal density estimate of model 2 residuals and model 2 residual plots. 
...
Data is negatively skewed. Reflected the distribution, by substracting values from a constant larger than the largest value, removed technical error control and three potential pipetting anomalies (control-1-8, imidacloprid-0.01-8, flupyradifurone-0.01-8 <- these three had an unusually long lag phase, possibly because not enough yeast culture was added initially) and log transformed. Still not normally distributed. Looking at the residual plots of the transformed data, the previous heteroscedasticity has been removed. There are 5 outliers. After inspecting the individual growth curves I see no technical reason to exclude them. The replicates were clothianidin-0.1-8, control-1-5, thiacloprid-1-6, malathion-5-3, acetamiprid-5-6. They all had large values but didn't appear to be outliers from visual inspection. Once removed the transformation works and the data is finally normally distributed. 

```{r}
# code below returns auc_l of residual outliers

#large_residuals <- c("clothianidin-0.1-8", "control-1-5", "thiacloprid-1-6", "malathion-5-3", "acetamiprid-5-6")

#large_residuals_data <- tibble()

#for (sample in 1:length(large_residuals)) {
  
#  row <- model_df_assigned_controls [grep(pattern = large_residuals [sample], x = model_df_assigned_controls$sample), ]
  
#  large_residuals_data <- rbind(row, large_residuals_data)
# }
```
...
log_aucl <- model_df_assigned_controls

# remove the outlier control. Go back and ensure all outliers have been correctly removed. Define formal criterior for removal process
log_aucl <-  log_aucl [-grep(pattern = "control-5-7", x = log_aucl$sample), ]

# remove potential pipetting error samples (not enough yeast culture added)
pipetting_anomalies <- c("control-1-8", "imidacloprid-0.01-8", "flupyradifurone-0.01-8")

for (sample in 1:length(pipetting_anomalies)) {
  
  log_aucl <-  log_aucl [-grep(pattern = pipetting_anomalies [sample], x = log_aucl$sample), ]
  
}

# remove 5 residual outliers identified from model constructed from log_aucl as defined above
large_residuals <- c("clothianidin-0.1-8", "control-1-5", "thiacloprid-1-6", "malathion-5-3", "acetamiprid-5-6")

for (sample in 1:length(large_residuals)) {
  
   log_aucl <- log_aucl [-grep(pattern = large_residuals [sample], x = log_aucl$sample), ]
  
}

positive_skew_reflection <- 1000-(log_aucl$auc_l)
...
log_aucl$auc_l <- positive_skew_reflection

log_aucl$auc_l <- log10(log_aucl$auc_l)

model_3 <- lm(auc_l ~ insecticide + concentration + day + insecticide*concentration, data = log_aucl)
...
# Inspect the model diagnostic metrics
model_3_metrics <- augment(model_3)

shapiro_test(model_3_metrics$.resid)

plot(density(model_df_assigned_controls$auc_l))

plot(density(log_aucl$auc_l))
...
# looks like a pretty good approximation apart from the at the distribution tail (QQ-plot). 
```
