...
Above I blindly decided to calculate the AUC mean. I don't know if my data is normally distributed. Visual inspection of day 8 replicates suggests that AUC may have been lower for all replicates on that day for some unknown reason (lower starting yeast concentration). If this is the case would this have skewed the data to the left (negative skew), where the mean would tend to be lower than the median. Median may be a better measure of central tendency. Let's look at the distribution of the data. 
...
# set levels to match each insecticide concentration. 
replicate_same_names <- replicate_same_names$...1

unique_replicate_names <- unique(replicate_same_names)

sample_aucl <- sample_aucl %>%
  reorder_levels(sample, order = unique_replicate_names)

# Build the linear model
model  <- lm(auc_l ~ sample, data = sample_aucl)
...
my data is non-normal. Should I be doing all insecticides at once or one by one. Isn't each set of insecticide concentrations an independent variable?
