...
There is, however, a nearly balanced incomplete block design. The first output shows the block design and the second is a concurrence matrix and shows how many times any pair of treatments appears together in the same block. 
...
```{r}
only_controls <- sample_aucl[grep("control", sample_aucl$insecticide), ]

# if i remove control again remember to change rep(7, each = 5) to rep(7, each = 4)
only_controls$day_numerical <- c(rep(c(1:6), each = 5), rep(7, each = 5), rep(8, each = 5))

ancova_df <- sample_aucl[-grep("control", sample_aucl$insecticide), ]
...
Remember that when you run the chunk above you randomly assign the controls to the insecticides again according to the IBD. See if doing this varies the results of the ANCOVA. Would show the control assignment does have an effect. Could do because I'm using a nearly balanced block design.
...
These are the only two models that mean anything biologically. Insecticide and concentration only mean something when taken together. Out of interest, when the interaction between all three independent variables is modelled Thiacloprid is no longer significant. Fewer degrees of freedom though. Adjusted R-squared seems alot higher. 

```{r}
model_3 <- lm(auc_l ~ insecticide + concentration + day + insecticide*concentration*day, data = ancova_df)
