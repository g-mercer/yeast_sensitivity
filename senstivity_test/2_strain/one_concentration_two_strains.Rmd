...
Of course there are loads of unnecessary comparisons going on here so extract the meaningful ones and perform a FDR correction. 

```{r}
post_hoc_tests <- summary(glht(mixed_model_1, linfct=mcp(insecticidestrain="Tukey")))
# extract the relevant information. This output matches the mix model contrasts output for all columns (standard error, t stat, p value)
post_hoc_test_results <- as.data.frame(post_hoc_tests [[10]] [3:6])

# extract relevant columns
search_terms <- c("- controlneg-pdr", "- controlneg-by")

relevant_comparisons_combined <- tibble()
  
for (i in 1:length(search_terms)) {
  
  relevant_comparisons <- post_hoc_test_results [grep(pattern = search_terms [i], x = rownames(post_hoc_test_results)), ]

  relevant_comparisons_combined <- rbind(relevant_comparisons_combined, relevant_comparisons)
}

regex_terms <- c("[[:alpha:]]+-pdr - controlneg-pdr", "[[:alpha:]]+-by - controlneg-by")

final_comparisons <- tibble()

for (i in 1:length(regex_terms)) {
  
strain_only <- relevant_comparisons_combined [grep(pattern = regex_terms [i], x = rownames(relevant_comparisons_combined)), ]

final_comparisons <- rbind(final_comparisons, strain_only)

}
```

P values above have been corrected already (wrong because too many comparisons) and not all the comparisons are the correct way around (control vs x). 
...
```{r}
# the easiest way would be to split the data by strain and analyse it separately. Alters standard error and t value slightly. Do it the above way. 

# this is easy but wrong. It is better to do what I did above, which is slightly alter how the model is determined and perform the
